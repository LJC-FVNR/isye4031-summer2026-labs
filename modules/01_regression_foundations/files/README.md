...
For homework-style work, submit both the completed `.ipynb` notebook and a PDF report. After running all cells, use `File` -> `Save and Export Notebook As` -> `PDF` to download a report view. Complete the student information table before exporting. If screenshots or uploaded images should appear in the PDF, display them through the provided `IPython.display.Image` code cell pattern. If the PDF option is unavailable in your browser, use the notebook interface and print the page to PDF.
